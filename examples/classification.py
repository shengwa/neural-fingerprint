...
task_params = {#'target_name' : 'measured log solubility in mols per litre',
               'target_name' : 'label', 
               'data_file'   : '../data/NT/NT_shuffled.csv'}
N_train = 218775
N_val   = 27347
N_test  = 27347
...
model_params = dict(fp_length=50,    # Usually neural fps need far fewer dimensions than morgan.
                    ...
                    L2_reg=np.exp(-2))
train_params = dict(num_iters=50000,
                    batch_size=100,
                    init_scale=np.exp(-6),
                    step_size=np.exp(-8))
...
def accuracy(preds, targets):
    ...
    return 1.0 * sum((preds > 0.5) == targets) / preds.shape[0]
...
vanilla_net_params = dict(
    ...
    normalize=True, L2_reg = model_params['L2_reg'], nll_func = binary_classification_nll)
...
def main():
    ...
    def print_classification_performance(pred_func):
        train_preds = pred_func(train_inputs)
        val_preds = pred_func(val_inputs)
        print "\nClassification Accuracy on " + task_params['target_name'] + ":"
        print "Train: ", accuracy(train_preds, train_targets)
        print "Test: ", accuracy(val_preds,  val_targets)
        print "-" * 80
        return accuracy(val_preds,  val_targets)
    ...
    def run_morgan_experiment():
        ...
        return print_classification_performance(predict_func)
    ...
    print
    # print "Morgan test accuracy:", test_loss_morgan, "Neural test accuracy:", test_loss_neural
